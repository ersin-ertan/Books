...
//    val intAsString:String = showInt.show(234)
  ...
  val showInt = Show.invoke<Int> { toString() }
...
//    can define custom instance of show like so, these are working
  val dateShow: Show<Date> = object : Show<Date> {
    override fun Date.show(): String = "${time}ms since epoch"
  }
  // or constructor lambda
  val dateShow2: Show<Date> = Show({ "${time}ms since epoch" })
  ...
  val intShow: Show<Int> = Show { toString() }
  val intAsString = intShow.apply { 234.show() }

  // try using run

  // 4 re implement cat using show
  val catShow = Show<Cat> { "$name is a $age year old $color cat" }
